...
class PurchaseDocumentUpdateDTO[TItem: BusinessDocumentItemBaseUpdateDTO](
    BasePydanticModel
):
    """Full-state update. ``shipped_at`` and the storage side (``debit_id``, the receiving storage) are editable
